// ============================================================================
// GRID / MARTINGALE SR ZONE BACKTEST — VERSION 3
// ============================================================================
// NEW in v3 (fixes from v2 analysis — fees were killing profit):
//  • TP now 2 grid levels away instead of 1 → doubles gross per trade, same fee
//  • Commission reduced to 0.02% (Asterdex limit = maker rate, not 0.05% taker)
//  • minGridStepPct filter — skip entry if grid spacing < 0.5% (fee not worth it)
//  • Wider test matrix: more HTF combos with the fixed params
// ============================================================================
...
class RunConfig {
  ...
  final int tpLevelsAway;       // how many grid levels to target for TP (v3: 2)
    ...
    for (final c in execBars) {
      ...
      if (grid.openLevels.length < cfg.maxOpenLevels) {
        for (int lvl = 0; lvl < levels.length - 1; lvl++) {
          ...
          // v3: skip if grid step is too small to cover fees
          ...
          // v3: TP is tpLevelsAway grid levels instead of 1
          final tpIdx = dir == GridDir.long
              ? lvl + cfg.tpLevelsAway
              : lvl - cfg.tpLevelsAway;
