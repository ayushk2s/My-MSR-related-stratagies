// ============================================================================
// STRATEGY HUNT — v8 (optimised: pre-bucketed 5m windows, O(n))
// ============================================================================
// Tests 13 strategy variants × ALL 21 assets × 4 timeframes.
...
//   3. SL wins when TP+SL both hit same 5m bar
//   4. Funding 0.01%/8h
...
//   Reused across all 13 strategy calls → ~10 000× faster than linear scan.
// ============================================================================
...
List<List<Candle>> _bucket(List<Candle> htf, List<Candle> exec) {
  ...
  int h = 0;
  for (final c in exec) {
    // Advance h until we're in the right window
...
//             → used during htf[i+1]'s execution window (Fix 1)
// tpAtr=0: ride until reverse signal (no fixed TP)
// ─────────────────────────────────────────────────────────────────────────────
...
Res _backtest(
  ...
  for (int bar = 0; bar < htf.length; bar++) {
    // Fix 1: use signal from previous closed bar
    ...
    for (int ei = 0; ei < bars.length; ei++) {
      ...
      if (active != null && active!.open) {
        ...
        final reverseSignal = sig == -t.dir && sig != 0;

        String rsn = ''; double at = 0; bool closed = false;
        if (slH || (tpH && slH)) { rsn = 'SL'; at = t.sl;   closed = true; }
        else if (tpH)             { rsn = 'TP'; at = t.tp;   closed = true; }
        else if (reverseSignal)   { rsn = 'FL'; at = c.close; closed = true; }
        ...
        if (closed) {
          ...
          final costs = _notional * _leverage * (_commission + _slippage) / 100 * 2 + t.fundPaid;
          netEq += gp - costs;
          if (rsn == 'SL' || rsn == 'FL') {
      ...
      if (active != null && active!.open) {
        ...
        op = raw - _notional * _leverage * (_commission + _slippage) / 100 * 2 - t.fundPaid;
      }
      final cur = netEq + op;
  ...
  if (active != null && active!.open) {
    final t = active!;
    t.open = false; t.exitP = windows.last.isNotEmpty ? windows.last.last.close : t.entry;
    t.reason = 'END';
    final gp = t.dir == 1
        ? (t.exitP - t.entry) * t.qty * _leverage
        : (t.entry - t.exitP) * t.qty * _leverage;
    netEq += gp - _notional * _leverage * (_commission + _slippage) / 100 * 2 - t.fundPaid;
  }
...
void main() {
  ...
  for (final (sym, p15, p5) in assets) {
    ...
    for (final (mult, htfLbl) in htfDefs) {
      ...
      // ── Pre-bucket 5m candles into HTF windows ONCE ──────────────────────
      ...
      // S3: Donchian(20) — breakout signal (momentary, not sustained)
      ...
      // Donchian sustained: once triggered, stay in direction until opposite breakout
      ...
      // S6: SFI + Donchian20 sustained combo
  ...
  if (allResults.isEmpty) {
    log('No profitable configs. Strategy hunt exhausted.');
    _writeReport(buf);
    return;
  }
  ...
  final sep = '═' * 100;
  log('╔$sep╗');
  log('║${_c('PROFITABLE CONFIGS — ALL ASSETS × HTF × STRATEGIES (bias-free)', 100)}║');
  log('╠════╦═══════════════╦══════════════════════════╦══════╦══════╦═════════╦═══════╦════════╦══════════════╣');
