...
extension TimeframeExt on Timeframe {
  int    get minutes => const {Timeframe.m5:5,Timeframe.m15:15,Timeframe.m30:30,Timeframe.h1:60,Timeframe.h4:240}[this]!;
  String get label   => const {Timeframe.m5:'5m',Timeframe.m15:'15m',Timeframe.m30:'30m',Timeframe.h1:'1h',Timeframe.h4:'4h'}[this]!;
}
...
({bool doExit, String reason, double exitIdeal}) _checkExit(
  ...
  if (s.dir == TradeDirection.long) {
    ...
    // Trailing stop update (only when no exit, so it does not interfere)
    if (!doExit) {
  ...
  List<Trade> backtest({
    ...
    for (int i = max(p.slow + 10, 60); i < c15.length - 1; i++) {
      ...
      if (!s.inTrade) {
        ...
        if (volOk && adxOk && trend4hL && rsiZoneL && crossL) {
          ...
          s.entryBar   = i + 1;
          s.beSet      = false;
        } else if (volOk && adxOk && trend4hS && rsiZoneS && crossS) {
  ...
  List<Trade> backtest({
    ...
    for (int i = p.chPeriod + 10; i < c15.length - 1; i++) {
      ...
      if (!s.inTrade) {
        ...
        final flipL  = d1h == 1 && d1hP == -1;  // 1h chandelier just flipped bullish
        final flipS  = d1h == -1 && d1hP == 1;  // 1h chandelier just flipped bearish
...
╔══════════════════════════════════════════════════════════════════════════════╗
║   CRYPTO TREND-FOLLOWING · WALK-FORWARD OPTIMIZER v3 (QUALITY-FIRST)        ║
║   ADX gate · Fixed targets · Break-even · Vol filter · IS PASS GATE         ║
║   IS pass gate: skip OOS if IS expectancy ≤ 0 OR IS trades < 10             ║
╚══════════════════════════════════════════════════════════════════════════════╝
''');
  }
...
''');
  ...
  static void exportCsv(
    ...
    final lb   = File('$outDir/leaderboard_v3.csv');
    ...
    final tr   = File('$outDir/all_trades_v3.csv');
    ...
    final wb   = File('$outDir/window_breakdown_v3.csv');
    ...
    print('  📄 leaderboard_v3.csv');
    print('  📄 all_trades_v3.csv');
    print('  📄 window_breakdown_v3.csv');
...
void main(List<String> args) {
  ...
  final outDir  = args.length > 1 ? args[1] : './wf_results_v3';
  ...
  // Compression-Breakout dropped — 20 wins vs 145 losses across all symbols,
  // and generates too many low-quality trades that eat fees.
