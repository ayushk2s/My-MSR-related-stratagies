...
const int kMaxBarsInTrade = 24 * 7; // v10: 7 days on 1h bars (1h entry TF)
...
class TrendPullbackEma extends Strategy {
  ...
  List<Trade> backtest({
    ...
    for (int i = max(p.slow + 10, 60); i < c15.length - 1; i++) {
      ...
      if (adx4h.adx[i4h].isNaN || e20_1d[i1d].isNaN || e20_1d[i1d - 1].isNaN)
        continue;
...
// ════════════════════════════════════════════════════════════════════════════
// v10 REBUILD: ChandelierMtf now uses 1h bars as the entry timeframe.
//
//  Why 1h instead of 15m?
//  ─────────────────────────────────────────────────────────────────────────
//  Analysis showed 15m EMA-cross and chandelier-flip signals are too noisy
//  (win rates 25-35%).  Moving one TF up to 1h produces cleaner signals —
//  each flip represents ~4× more price action and is far less likely to be
//  a whipsaw.  Fewer trades, higher win rate, lower fee drag.
//
//  Architecture
//  ─────────────────────────────────────────────────────────────────────────
//  Main loop   : 1h bars  (entry, exit, trailing all on 1h)
//  Trigger     : 4h chandelier FLIPS bullish/bearish  (the setup)
//  Confirm     : 1h chandelier already agrees + 1h close > EMA50_1h
//  Macro gate  : daily close > EMA20_daily  (primary trend aligned)
//  Strength    : 4h ADX > adxMin (25 or 30 — raised from v6's 20)
//  Volume      : 1h bar volume > 1.2× SMA20(volume) on signal bar
//  Stop        : 2×ATR(1h) initial; chandelier(1h) trailing; BE at +1R
//  Target      : grid of 2×–3× R:R (= 4–6×ATR)
//  Max hold    : kMaxBarsInTrade (7 days on 1h)
// ════════════════════════════════════════════════════════════════════════════
...
class ChandelierMtf extends Strategy {
  ...
  List<StrategyParams> grid() {
    final out = <StrategyParams>[];
    for (final per in [10, 14, 22]) {
      for (final m in [2.0, 2.5, 3.0]) {
        for (final adx in [25, 30]) {   // v10: raised floor from 20 → 25
          for (final rr in [2.0, 2.5, 3.0]) {
            out.add(ChandelierParams(per, m, adx, 1.2, 2.0, 2.0 * rr));
          }
        }
      }
    }
    return out; // 54 combos — thorough IS search
  }
  ...
  List<Trade> backtest({
    ...
    // ── Timeframe data ───────────────────────────────────────────────────────
    final c1h = tfCandles[Timeframe.h1]!;   // main loop TF (was 15m in v6)
    final c4h = tfCandles[Timeframe.h4]!;   // trigger TF  (was 1h in v6)
    final c1d = tfCandles[Timeframe.d1]!;   // macro gate

    // ── 1h indicators (entry + trailing) ────────────────────────────────────
    ...
    // ── 4h indicators (state, ADX strength, EMA alignment) ─────────────────
    final ch4h   = Indicators.chandelier(c4h, p.chPeriod, p.chMult);
    final adx4h  = Indicators.adx(c4h, 14);
    final cl4h   = c4h.map((c) => c.close).toList();
    final e20_4h = Indicators.ema(cl4h, 20);   // v11: 4h EMA20
    final e50_4h = Indicators.ema(cl4h, 50);   // v11: 4h EMA50
    ...
    // ── Main loop on 1h bars ─────────────────────────────────────────────────
    for (int i = p.chPeriod + 10; i < c1h.length - 1; i++) {
      if (atr1h[i].isNaN || e50_1h[i].isNaN || volSma1h[i].isNaN) continue;

      final t   = c1h[i].time;
      final i4h = Indicators.closedHtfIdx(c4h, t);
      final i1d = Indicators.closedHtfIdx(c1d, t);
      if (i4h < 1 || i1d < 0) continue;
      if (adx4h.adx[i4h].isNaN || e20_1d[i1d].isNaN) continue;
      if (e20_4h[i4h].isNaN || e50_4h[i4h].isNaN) continue;

      // Skip oversized candles (gap opens, news spikes)
      final barRange = c1h[i].high - c1h[i].low;
      if (barRange > 2.0 * atr1h[i]) continue;

      if (!s.inTrade) {
        // ── Entry conditions ─────────────────────────────────────────────────
        final adxOk  = adx4h.adx[i4h] > p.adxMin;   // trend strong enough
        final volOk  = c1h[i].volume > p.volFactor * volSma1h[i]; // volume

        // Macro: daily close must be above/below EMA20
        final dBull = c1d[i1d].close > e20_1d[i1d];
        final dBear = c1d[i1d].close < e20_1d[i1d];

        // Trigger: 1h chandelier FLIPS (same frequency as v6 trigger)
        final d1h     = ch1h.dir[i];
        final d1hPrev = i > 0 ? ch1h.dir[i - 1] : d1h;
        final flip1hL = d1h == 1 && d1hPrev == -1;   // 1h just turned bullish
        final flip1hS = d1h == -1 && d1hPrev == 1;   // 1h just turned bearish

        // Trend filter: 4h chandelier state confirms direction
        final d4h      = ch4h.dir[i4h];
        final trend4hL = d4h == 1;    // 4h chandelier is bullish
        final trend4hS = d4h == -1;   // 4h chandelier is bearish

        // v11: 4h EMA alignment — EMA20 above/below EMA50 confirms 4h structure
        final ema4hBull = e20_4h[i4h] > e50_4h[i4h];  // 4h uptrend structure
        final ema4hBear = e20_4h[i4h] < e50_4h[i4h];  // 4h downtrend structure

        // Confirm: 1h close on correct side of EMA50
        final conf1hL = c1h[i].close > e50_1h[i];
        final conf1hS = c1h[i].close < e50_1h[i];

        if (adxOk && volOk && dBull && flip1hL && trend4hL && ema4hBull && conf1hL) {
          s.dir        = TradeDirection.long;
          ...
          s.inTrade    = true;
          s.entryTime  = c1h[i + 1].time;
          s.entryBar   = i + 1;
          s.beSet      = false;
        } else if (adxOk && volOk && dBear && flip1hS && trend4hS && ema4hBear && conf1hS) {
          s.dir        = TradeDirection.short;
          s.entryPrice = _entryFill(c1h[i + 1].open, s.dir);
          s.origStop   = s.entryPrice + p.atrStopMult * atr1h[i];
          s.stopLoss   = s.origStop;
        ...
        // ── Chandelier trailing stop (1h, ratcheting) ───────────────────────
...
class WfoEngine {
  ...
  WfoEngine({
    this.isLengthDays = 60,
    this.oosLengthDays = 30,
    this.minIsTrades = 10,
  });
...
class Reporter {
  ...
  static void banner() {
    print('''
╔══════════════════════════════════════════════════════════════════════════════╗
║   CRYPTO TREND-FOLLOWING · v11  (MTF CONFLUENCE)                              ║
║   Single strategy: Chandelier-MTF on 1h entry timeframe                       ║
║   Trigger: 1h chandelier FLIP  |  4h state + 4h EMA20>EMA50 align            ║
║   Macro gate: daily close > EMA20  |  ADX 4h > 25 minimum                    ║
║   WFO IS=60d / OOS=30d sliding, covers all data                               ║
...
''');
  ...
  static void exportCsv(
    ...
    final lb = File('$outDir/leaderboard_v11.csv');
    ...
    final tr = File('$outDir/all_trades_v11.csv');
    ...
    final wb = File('$outDir/window_breakdown_v11.csv');
    ...
    print('  📄 leaderboard_v11.csv');
    print('  📄 all_trades_v11.csv');
    print('  📄 window_breakdown_v11.csv');
...
void main(List<String> args) {
  ...
  final outDir = args.length > 1 ? args[1] : './wf_results_v11';
  ...
  // v11: single strategy — Chandelier-MTF with 4h EMA confluence added.
  // All other strategies removed: Chandelier-MTF is the only one with
  // consistent edge across both WFO and full-backtest evaluation.
  final strategies = <Strategy>[ChandelierMtf()];
