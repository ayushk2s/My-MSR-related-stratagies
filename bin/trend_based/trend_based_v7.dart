// ════════════════════════════════════════════════════════════════════════════
//  CRYPTO TREND-FOLLOWING · WALK-FORWARD OPTIMIZER v7
//
//  v7 improvements over v6
//  ────────────────────────────────────────────────────────────────────────────
//  1. ChandelierMtf — no fixed profit target; exit when 1h chandelier flips.
//     v6's fixed TP capped winners early. Now the trade rides the full 1h
//     trend: entry on 1h flip bullish, exit on 1h flip bearish.
//
//  2. ChandelierMtf — 4h ADX must be RISING at entry. Entering only when
//     the trend is strengthening (not decaying) filters out late-cycle fades.
//
//  3. NEW strategy: EmaCross-Rider — enters when 1h EMA(fast) crosses above
//     EMA(slow) while 4h macro agrees and 4h ADX is rising. Exits on the
//     reverse 1h EMA cross (death cross). Structurally different from
//     Chandelier (ATR-based) — adds a price-momentum angle.
//
//  4. Dropped: KAMA-Pullback (consistently negative across all symbols) and
//     Supertrend-Quality (0-4 trades per asset, statistically void).
//
//  Retained from v6
//  ────────────────────────────────────────────────────────────────────────────
//  • ATR trailing stop removed from _checkExit (chandelier stop handles trail)
//  • Daily EMA20 slope gate on TrendPullback-EMA
//  • IS gate: expectancy > 0 AND trades >= 10
//  • Break-even stop at +1R
//  • ADX regime gate + volume filter on all strategies
// ════════════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════════════
// STRATEGY 2 — CHANDELIER MTF  (v7)
// ════════════════════════════════════════════════════════════════════════════
//
//  v7 changes vs v6
//  • Larger fixed TP grid: 3.0–5.0× ATR (was 4–5× in v6). Bigger R:R.
//  • 4h ADX must be RISING at entry — enter only on strengthening momentum.
//  • 1h chandelier flip is a SECONDARY exit — if TP not yet hit and 1h flips
//    against the trade, exit early to protect profits.
//  • ADX threshold lowered to 18-22 from 20-25 to allow more valid entries.
//
//  Entry    4h chandelier bullish + 4h ADX > adxMin AND rising.
//           1h chandelier flips bullish (entry trigger).
//           15m EMA50 confirms direction + 15m chandelier agrees.
//           15m volume > volFactor × SMA20(vol).
//  Exit     Fixed TP (primary). 1h flip (secondary). 15m chandelier trailing
//           stop. Break-even at +1R. Time stop 5 days.
// ════════════════════════════════════════════════════════════════════════════
...
class ChandelierMtf extends Strategy {
    ...
    for (final per in [14, 22]) {
      for (final m in [2.5, 3.0]) {
        for (final adx in [18, 22]) {
          for (final tp in [3.0, 4.0, 5.0]) {
            out.add(ChandelierParams(per, m, adx, 1.2, 2.0, tp));
          }
        }
      }
    }
    return out; // 24 combos
  }
  ...
  List<Trade> backtest({
    ...
    for (int i = p.chPeriod + 10; i < c15.length - 1; i++) {
      ...
      final i1h = Indicators.closedHtfIdx(c1h, t);
      final i4h = Indicators.closedHtfIdx(c4h, t);
      if (i1h < 1 || i4h < 1) continue;
      if (adx4h.adx[i4h].isNaN || adx4h.adx[i4h - 1].isNaN || adx1h.adx[i1h].isNaN) continue;

      final barRange = c15[i].high - c15[i].low;
      if (barRange > 1.5 * atr15[i]) continue;

      if (!s.inTrade) {
        // v7: require 4h ADX to be rising (trend strengthening, not fading)
        final adxOk     = adx4h.adx[i4h] > p.adxMin;
        final adxRising = adx4h.adx[i4h] > adx4h.adx[i4h - 1];
        final volOk     = c15[i].volume > p.volFactor * volSma15[i];
        ...
        if (adxOk && adxRising && volOk && d4h == 1 && flipL && conf15L) {
          s.dir        = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop   = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss   = s.origStop;
        ...
        } else if (adxOk && adxRising && volOk && d4h == -1 && flipS && conf15S) {
          s.dir        = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop   = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss   = s.origStop;
        ...
        // Trail stop with 15m chandelier (ratchets in profit direction)
        ...
        // v7 secondary exit: 1h chandelier flips against position (early profit protect)
        final d1hNow  = ch1h.dir[i1h];
        final d1hPrev = ch1h.dir[i1h - 1];
        final flip1hExit = (s.dir == TradeDirection.long  && d1hNow == -1 && d1hPrev == 1)
                         || (s.dir == TradeDirection.short && d1hNow == 1  && d1hPrev == -1);

        final ex = _checkExit(s, c15[i], atr15[i], p.atrStopMult);
        var doExit = ex.doExit; var reason = ex.reason; var exitIdeal = ex.exitIdeal;
        if (!doExit && flip1hExit) {
          doExit = true; reason = '1h Flip'; exitIdeal = c15[i].close;
        }
        if (!doExit && i - s.entryBar >= kMaxBarsInTrade) {
          doExit = true; reason = 'Time Stop'; exitIdeal = c15[i].close;
        }
...
// ════════════════════════════════════════════════════════════════════════════
// STRATEGY 3 — EMA CROSS RIDER  (new in v7)
// ════════════════════════════════════════════════════════════════════════════
//
//  Concept  Enter on 1h EMA golden/death cross confirmed by 4h macro trend
//           and rising ADX. Exit on the reverse cross. Structurally different
//           from ChandelierMtf (price-momentum vs. ATR channel flip).
//  Entry    4h EMA20 > EMA50 (macro bull) AND 4h ADX > adxMin AND rising.
//           1h EMA(fast) just crossed above EMA(slow) → momentum ignition.
//           15m volume > volFactor × SMA20(vol).
//  Exit     1h EMA(fast) crosses below EMA(slow) (death cross for longs).
//           OR 15m ATR stop hit. OR break-even stop. OR time stop 5 days.
// ════════════════════════════════════════════════════════════════════════════

class EmaCrossParams extends StrategyParams {
  final int    fastEma, slowEma, adxMin;
  final double volFactor;
  @override final double atrStopMult, atrTargetMult;
  EmaCrossParams(this.fastEma, this.slowEma, this.adxMin,
      this.volFactor, this.atrStopMult, this.atrTargetMult);
  @override String get label =>
      'EMA1h($fastEma/$slowEma) ADX>$adxMin SL×$atrStopMult TP×$atrTargetMult';
}

class EmaCrossRider extends Strategy {
  @override String get name => 'EmaCross-Rider';

  @override
  List<StrategyParams> grid() {
    final out = <StrategyParams>[];
    for (final pair in [(9, 21), (12, 26)]) {
      for (final adx in [18, 22]) {
        for (final tp in [2.5, 3.5]) {
          out.add(EmaCrossParams(pair.$1, pair.$2, adx, 1.2, 1.5, tp));
        }
      }
    }
  ...
  List<Trade> backtest({
    ...
    final p   = params as EmaCrossParams;
    final c15 = tfCandles[Timeframe.m15]!;
    final c1h = tfCandles[Timeframe.h1]!;
    final c4h = tfCandles[Timeframe.h4]!;
    ...
    final cl1h = c1h.map((c) => c.close).toList();
    final eF1h = Indicators.ema(cl1h, p.fastEma);
    final eS1h = Indicators.ema(cl1h, p.slowEma);
    ...
    for (int i = max(p.slowEma + 10, 60); i < c15.length - 1; i++) {
      if (atr15[i].isNaN || volSma15[i].isNaN) continue;
      final t   = c15[i].time;
      final i1h = Indicators.closedHtfIdx(c1h, t);
      final i4h = Indicators.closedHtfIdx(c4h, t);
      if (i1h < 1 || i4h < 1) continue;
      if (eF1h[i1h].isNaN || eS1h[i1h].isNaN ||
          eF1h[i1h - 1].isNaN || eS1h[i1h - 1].isNaN) continue;
      if (e20_4h[i4h].isNaN || e50_4h[i4h].isNaN ||
          adx4h.adx[i4h].isNaN || adx4h.adx[i4h - 1].isNaN) continue;

      final adxOk     = adx4h.adx[i4h] > p.adxMin;
      final adxRising = adx4h.adx[i4h] > adx4h.adx[i4h - 1];
      final macro4hL  = e20_4h[i4h] > e50_4h[i4h];
      final macro4hS  = e20_4h[i4h] < e50_4h[i4h];
      final volOk     = c15[i].volume > p.volFactor * volSma15[i];

      // 1h EMA cross detection
      final eF1hNow  = eF1h[i1h];     final eS1hNow  = eS1h[i1h];
      final eF1hPrev = eF1h[i1h - 1]; final eS1hPrev = eS1h[i1h - 1];
      final crossBull = eF1hNow > eS1hNow && eF1hPrev <= eS1hPrev;
      final crossBear = eF1hNow < eS1hNow && eF1hPrev >= eS1hPrev;

      if (!s.inTrade) {
        if (adxOk && adxRising && macro4hL && crossBull && volOk) {
          s.dir        = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop   = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss   = s.origStop;
          s.takeProfit = s.entryPrice + p.atrTargetMult * atr15[i];
          s.inTrade    = true; s.entryTime = c15[i + 1].time; s.entryBar = i + 1; s.beSet = false;
        } else if (adxOk && adxRising && macro4hS && crossBear && volOk) {
          s.dir        = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop   = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss   = s.origStop;
        ...
        // EMA cross exit as secondary (stop/TP checked first in _checkExit)
        final emaExitL = s.dir == TradeDirection.long  && crossBear;
        final emaExitS = s.dir == TradeDirection.short && crossBull;

        final ex = _checkExit(s, c15[i], atr15[i], p.atrStopMult);
        var doExit = ex.doExit; var reason = ex.reason; var exitIdeal = ex.exitIdeal;
        if (!doExit && (emaExitL || emaExitS)) {
          doExit = true; reason = 'EMA Cross'; exitIdeal = c15[i].close;
        }
        if (!doExit && i - s.entryBar >= kMaxBarsInTrade) {
          doExit = true; reason = 'Time Stop'; exitIdeal = c15[i].close;
        }
...
class StQualityParams extends StrategyParams {
  ...
  @override final double atrStopMult, atrTargetMult;
  StQualityParams(this.period, this.mult, this.adxMin,
      this.volFactor, this.atrStopMult, this.atrTargetMult);
  @override String get label =>
...
class SupertrendQuality extends Strategy {
  @override String get name => 'Supertrend-Quality';
    ...
    for (int i = 80; i < c15.length - 1; i++) {
      ...
      if (!s.inTrade) {
        ...
        if (adxOk && volOk && d4h == 1 && d1h == 1 && d15 == 1 && d15p == -1) {
          ...
          s.takeProfit = s.entryPrice + p.atrTargetMult * atr15[i];
          s.inTrade    = true; s.entryTime = c15[i + 1].time; s.entryBar = i + 1; s.beSet = false;
        } else if (adxOk && volOk && d4h == -1 && d1h == -1 && d15 == -1 && d15p == 1) {
          s.dir        = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop   = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss   = s.origStop;
...
class CompressionParams extends StrategyParams {
  ...
  @override final double atrStopMult, atrTargetMult;
  CompressionParams(this.donPeriod, this.bbWidthMax, this.adxMin,
      this.volFactor, this.atrStopMult, this.atrTargetMult);
  @override String get label =>
...
class CompressionBreakout extends Strategy {
  @override String get name => 'Compression-Breakout';
  ...
  List<Trade> backtest({
    ...
    for (int i = max(p.donPeriod + 2, 30); i < c15.length - 1; i++) {
      if (don15.upper[i - 1].isNaN || atr15[i].isNaN || volSma15[i].isNaN) continue;
      final t   = c15[i].time;
      final i1h = Indicators.closedHtfIdx(c1h, t);
      final i4h = Indicators.closedHtfIdx(c4h, t);
      if (i1h < 0 || i4h < 0) continue;
      if (e50_4h[i4h].isNaN || adx4h.adx[i4h].isNaN || adx1h.adx[i1h].isNaN) continue;
      ...
      if (!s.inTrade) {
        ...
        final volOk   = c15[i].volume > p.volFactor * volSma15[i];
        final breakL  = c15[i].close > don15.upper[i - 1] && c15[i - 1].close <= don15.upper[i - 1];
        final breakS  = c15[i].close < don15.lower[i - 1] && c15[i - 1].close >= don15.lower[i - 1];
...
class KamaPullbackParams extends StrategyParams {
  ...
  final double rsiLow, rsiHigh, volFactor;
  @override final double atrStopMult, atrTargetMult;
  KamaPullbackParams(this.kamaPeriod, this.adxMin,
      this.rsiLow, this.rsiHigh, this.volFactor,
      this.atrStopMult, this.atrTargetMult);
  @override String get label =>
...
class KamaPullback extends Strategy {
  @override String get name => 'KAMA-Pullback';
    ...
    for (final kp in [14, 21]) {
      for (final adx in [18, 22]) {
        for (final zone in [(35.0, 60.0), (42.0, 65.0)]) {
          for (final rr in [2.0, 2.5]) {
            out.add(KamaPullbackParams(kp, adx, zone.$1, zone.$2, 1.2, 1.5, 1.5 * rr));
          }
        }
      }
    }
  ...
  List<Trade> backtest({
    ...
    for (int i = max(p.kamaPeriod + 20, 60); i < c15.length - 1; i++) {
      ...
      if (rsi1h[i1h].isNaN || kama4h[i4h].isNaN || kama4h[i4h - 5].isNaN) continue;
...
class WfoEngine {
  ...
  WfoEngine({this.isLengthDays = 60, this.oosLengthDays = 30, this.minIsTrades = 7});
...
╔══════════════════════════════════════════════════════════════════════════════╗
║   CRYPTO TREND-FOLLOWING · WALK-FORWARD OPTIMIZER v7                         ║
║   Chandelier: 1h-flip exit + ADX rising filter | New: EmaCross-Rider         ║
║   IS gate: expectancy > 0 AND trades >= 7                                    ║
╚══════════════════════════════════════════════════════════════════════════════╝
''');
  }
...
''');
  ...
  static void exportCsv(
    ...
    final lb   = File('$outDir/leaderboard_v7.csv');
    ...
    final tr   = File('$outDir/all_trades_v7.csv');
    ...
    final wb   = File('$outDir/window_breakdown_v7.csv');
    ...
    print('  📄 leaderboard_v7.csv');
    print('  📄 all_trades_v7.csv');
    print('  📄 window_breakdown_v7.csv');
...
void main(List<String> args) {
  ...
  final outDir  = args.length > 1 ? args[1] : './wf_results_v7';
  ...
  // v7: KAMA-Pullback and Supertrend-Quality dropped (consistently negative).
  // EmaCross-Rider added as a complementary momentum strategy.
