// ════════════════════════════════════════════════════════════════════════════
//  CRYPTO TREND-FOLLOWING · WALK-FORWARD OPTIMIZER v3 (QUALITY-FIRST)
//
//  Why v2 loses — root-cause analysis
//  ────────────────────────────────────────────────────────────────────────────
//  1. NO REGIME FILTER  All strategies fire in choppy / ranging markets where
//     trend-following is guaranteed to lose.  ADX existed but was never used.
//     Win rates of 22-34% with PF < 1 are the signature of a strategy that is
//     equally happy entering in trends and in noise.
//
//  2. NO PROFIT TARGET  The only exits were trailing stop or signal flip.
//     In noisy 15m data those flip constantly, giving back every gain before
//     it can compound.  Average winner < average loser → PF < 1.
//
//  3. FEE DRAG  18 bps round-trip × 600-900 trades = 100-160 % drag.
//     Any strategy that trades too often in choppy markets bleeds out.
//
//  4. IS OPTIMISATION BY COMPOSITE SCORE  The blended score rewarded win-rate
//     rather than expectancy, picking combos with many marginal wins that did
//     not transfer to OOS (classic overfit signature).
//
//  v3 Fixes
//  ────────────────────────────────────────────────────────────────────────────
//  • ADX regime gate (every strategy) — no trades when ADX < threshold.
//  • Fixed profit target (2–3× ATR) enforces minimum R:R on every trade.
//  • Break-even stop — once price moves +1R in our favour, stop moves to
//    entry price; a previously winning trade can never turn into a loss.
//  • Volume filter — entry bar volume must exceed 1.2–1.5× rolling average.
//  • IS optimisation by expectancy × √trades (penalises both bad edge AND
//    statistically thin results from too few trades).
//  • Extended OOS window (30 days) for more robust evaluation.
//  • Five redesigned strategies:
//      1. TrendPullback-EMA    RSI pullback in EMA + ADX uptrend
//      2. Supertrend-Quality   ADX + volume gated triple-TF ST
//      3. Compression-Breakout BBWidth squeeze → Donchian expansion
//      4. KAMA-Pullback        KAMA slope defines trend, RSI zones time entry
//      5. Chandelier-MTF       Chandelier Exit direction + ADX filter
// ════════════════════════════════════════════════════════════════════════════
...
double _entryFill(double px, TradeDirection d) => d == TradeDirection.long
    ? px * (1 + kSlippagePerSide)
    : px * (1 - kSlippagePerSide);
double _exitFill(double px, TradeDirection d) => d == TradeDirection.long
    ? px * (1 - kSlippagePerSide)
    : px * (1 + kSlippagePerSide);
...
class TrendPullbackParams extends StrategyParams {
  ...
  @override
  final double atrStopMult, atrTargetMult;
  TrendPullbackParams(
    this.fast,
    this.slow,
    this.adxMin,
...
class TrendPullbackEma extends Strategy {
  @override
  String get name => 'TrendPullback-EMA';
    ...
    for (final fast in [9, 13]) {
      for (final slow in [21, 34]) {
        for (final adx in [20, 25]) {
          // v9: slightly tighter RSI zone + 1.3x volume (was 1.2x)
          for (final rsiZone in [(38.0, 63.0), (40.0, 62.0)]) {
            for (final rr in [2.0, 2.5]) {
              out.add(
                TrendPullbackParams(
                  ...
                  rsiZone.$1,
                  rsiZone.$2,
                  1.3, // v9: 1.3x volume — moderate increase from 1.2x
                  1.5,
                  1.5 * rr,
                ),
              );
  ...
  List<Trade> backtest({
    ...
    // v9: 1h EMA alignment — trend must be confirmed on 1h too
    final e20_1h = Indicators.ema(cl1h, 20);
    ...
    for (int i = max(p.slow + 10, 60); i < c15.length - 1; i++) {
      ...
      if (e20_1h[i1h].isNaN || e50_1h[i1h].isNaN) continue;
      if (adx4h.adx[i4h].isNaN || e20_1d[i1d].isNaN) continue;
      ...
      if (!s.inTrade) {
        ...
        // v9: 1h trend alignment (all 3 TFs must agree: 4h + 1h + daily)
        final trend1hL = e20_1h[i1h] > e50_1h[i1h];
        final trend1hS = e20_1h[i1h] < e50_1h[i1h];
        final rsiZoneL = rsiVal >= p.rsiLow && rsiVal <= p.rsiHigh;
        final rsiZoneS =
            rsiVal <= (100 - p.rsiLow) && rsiVal >= (100 - p.rsiHigh);
        // v9: daily close vs EMA20 level (stable primary trend direction)
        final dBull = c1d[i1d].close > e20_1d[i1d];
        final dBear = c1d[i1d].close < e20_1d[i1d];

        // v9: require 1h + 4h + daily all aligned before entry
        if (volOk && adxOk && trend1hL && trend4hL && dBull && rsiZoneL && crossL) {
          ...
          s.entryBar = i + 1;
          s.beSet = false;
        } else if (volOk && adxOk && trend1hS && trend4hS && dBear && rsiZoneS && crossS) {
...
// ════════════════════════════════════════════════════════════════════════════
// STRATEGY 2 — SUPERTREND QUALITY
// ════════════════════════════════════════════════════════════════════════════
//
//  Concept  Triple-TF Supertrend alignment (same as v2) but only when
//           the market is genuinely trending — no more noise trades.
//  Entry    4h + 1h Supertrend aligned AND 1h ADX > threshold AND
//           15m ST flips in same direction AND volume spike.
//  Exit     Fixed target; trailing ATR stop; break-even; time stop.
//           ← no ST-flip exit (that was the churn source in v2)
// ════════════════════════════════════════════════════════════════════════════

class StQualityParams extends StrategyParams {
  final int period, adxMin;
  final double mult, volFactor;
  @override
  final double atrStopMult, atrTargetMult;
  StQualityParams(
    this.period,
  ...
  @override
  String get label =>
      'ST($period,${mult.toStringAsFixed(1)}) ADX>$adxMin '
      'Vol>×${volFactor.toStringAsFixed(1)} SL×$atrStopMult TP×$atrTargetMult';
}

class SupertrendQuality extends Strategy {
  @override
  String get name => 'Supertrend-Quality';

  @override
  List<StrategyParams> grid() {
    final out = <StrategyParams>[];
    for (final per in [10, 14]) {
      for (final m in [2.5, 3.0]) {
        for (final adx in [22, 27]) {
          for (final rr in [2.0, 2.5]) {
            out.add(StQualityParams(per, m, adx, 1.3, 2.0, 2.0 * rr));
          }
        }
      }
    }
  ...
  List<Trade> backtest({
    ...
    final p = params as StQualityParams;
    final c15 = tfCandles[Timeframe.m15]!;
    final c1h = tfCandles[Timeframe.h1]!;
    final c4h = tfCandles[Timeframe.h4]!;
    ...
    for (int i = 80; i < c15.length - 1; i++) {
      if (atr15[i].isNaN || volSma15[i].isNaN) continue;
      final t = c15[i].time;
      final i1h = Indicators.closedHtfIdx(c1h, t);
      final i4h = Indicators.closedHtfIdx(c4h, t);
      if (i1h < 0 || i4h < 0) continue;
      if (adx1h.adx[i1h].isNaN) continue;

      final barRange = c15[i].high - c15[i].low;
      if (barRange > 1.5 * atr15[i]) continue;

      if (!s.inTrade) {
        final adxOk = adx1h.adx[i1h] > p.adxMin;
        final volOk = c15[i].volume > p.volFactor * volSma15[i];
        ...
        if (adxOk && volOk && d4h == 1 && d1h == 1 && d15 == 1 && d15p == -1) {
          s.dir = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
...
// ════════════════════════════════════════════════════════════════════════════
// STRATEGY 3 — COMPRESSION BREAKOUT
// ════════════════════════════════════════════════════════════════════════════
//
//  Concept  Enter Donchian breakouts only AFTER a confirmed consolidation
//           phase (BB Width below threshold on 1h). Low-volatility periods
//           are followed by expansion — we catch the breakout, not the noise.
//  Entry    4h price > EMA50 + ADX > threshold (macro trend confirmed).
//           1h BB Width < bbWidthMax (market is coiling).
//           15m Donchian breakout with volume spike.
//  Exit     Fixed target (3R); ATR trailing stop; break-even; time stop.
// ════════════════════════════════════════════════════════════════════════════

class CompressionParams extends StrategyParams {
  final int donPeriod, adxMin;
  final double bbWidthMax, volFactor;
  @override
  final double atrStopMult, atrTargetMult;
  CompressionParams(
    this.donPeriod,
  ...
  @override
  String get label =>
      'Don($donPeriod) BBW<${bbWidthMax.toStringAsFixed(3)} ADX>$adxMin '
      'SL×$atrStopMult TP×$atrTargetMult';
}

class CompressionBreakout extends Strategy {
  @override
  String get name => 'Compression-Breakout';

  @override
  List<StrategyParams> grid() {
    final out = <StrategyParams>[];
    for (final don in [20, 40]) {
      for (final bbw in [0.02, 0.04]) {
        for (final adx in [18, 23]) {
          for (final rr in [2.5, 3.0]) {
            out.add(CompressionParams(don, bbw, adx, 1.5, 1.5, 1.5 * rr));
          }
        }
      }
    }
  ...
  List<Trade> backtest({
    ...
    final p = params as CompressionParams;
    final c15 = tfCandles[Timeframe.m15]!;
    final c1h = tfCandles[Timeframe.h1]!;
    final c4h = tfCandles[Timeframe.h4]!;
    ...
    final cl1h = c1h.map((c) => c.close).toList();
    final bb1h = Indicators.bollinger(cl1h, 20, 2.0);
    final adx1h = Indicators.adx(c1h, 14);
    ...
    for (int i = max(p.donPeriod + 2, 30); i < c15.length - 1; i++) {
      if (don15.upper[i - 1].isNaN || atr15[i].isNaN || volSma15[i].isNaN)
        continue;
      final t = c15[i].time;
      final i1h = Indicators.closedHtfIdx(c1h, t);
      final i4h = Indicators.closedHtfIdx(c4h, t);
      if (i1h < 0 || i4h < 0) continue;
      if (e50_4h[i4h].isNaN || adx4h.adx[i4h].isNaN || adx1h.adx[i1h].isNaN)
        continue;
      if (bb1h.upper[i1h].isNaN || bb1h.mid[i1h].isNaN) continue;

      if (!s.inTrade) {
        final bull4h = c4h[i4h].close > e50_4h[i4h];
        final bear4h = c4h[i4h].close < e50_4h[i4h];
        final adxOk = adx4h.adx[i4h] > p.adxMin || adx1h.adx[i1h] > p.adxMin;
        // BB Width = (upper - lower) / mid — low value means market is coiling
        final bbWidth = (bb1h.upper[i1h] - bb1h.lower[i1h]) / bb1h.mid[i1h];
        final coiling = bbWidth < p.bbWidthMax;
        final volOk = c15[i].volume > p.volFactor * volSma15[i];
        final breakL =
            c15[i].close > don15.upper[i - 1] &&
            c15[i - 1].close <= don15.upper[i - 1];
        final breakS =
            c15[i].close < don15.lower[i - 1] &&
            c15[i - 1].close >= don15.lower[i - 1];

        if (adxOk && coiling && volOk && bull4h && breakL) {
          s.dir = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
          ...
          s.entryBar = i + 1;
          s.beSet = false;
        } else if (adxOk && coiling && volOk && bear4h && breakS) {
          s.dir = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
...
// ════════════════════════════════════════════════════════════════════════════
// STRATEGY 4 — KAMA PULLBACK
// ════════════════════════════════════════════════════════════════════════════
//
//  Concept  KAMA (Kaufman Adaptive MA) barely moves in choppy markets and
//           accelerates in trends.  We use 4h KAMA slope to define the trend
//           and wait for the 1h RSI to pull back to a "continuation zone"
//           before triggering a 15m entry.  Entries happen at better prices
//           than pure breakout strategies → higher win rate.
//  Entry    4h KAMA rising (KAMA[i] > KAMA[i-5]) for long.
//           4h ADX(14) > adxMin.
//           1h RSI(14) in pullback zone [rsiLow, rsiHigh].
//           15m EMA(9) crosses above EMA(21) (momentum resuming).
//           15m volume > volFactor × SMA20(vol).
//  Exit     Fixed target; trailing ATR stop; break-even; time stop.
// ════════════════════════════════════════════════════════════════════════════

class KamaPullbackParams extends StrategyParams {
  final int kamaPeriod, adxMin;
  final double rsiLow, rsiHigh, volFactor;
  @override
  final double atrStopMult, atrTargetMult;
  KamaPullbackParams(
  ...
  @override
  String get label =>
      'KAMA($kamaPeriod) ADX>$adxMin RSI[$rsiLow,$rsiHigh] SL×$atrStopMult TP×$atrTargetMult';
}

class KamaPullback extends Strategy {
  @override
  String get name => 'KAMA-Pullback';

  @override
  List<StrategyParams> grid() {
    final out = <StrategyParams>[];
    for (final kp in [14, 21]) {
      for (final adx in [18, 22]) {
        for (final zone in [(35.0, 60.0), (42.0, 65.0)]) {
          for (final rr in [2.0, 2.5]) {
            out.add(
              KamaPullbackParams(kp, adx, zone.$1, zone.$2, 1.2, 1.5, 1.5 * rr),
            );
          }
        }
      }
  ...
  List<Trade> backtest({
    ...
    final p = params as KamaPullbackParams;
    final c15 = tfCandles[Timeframe.m15]!;
    final c1h = tfCandles[Timeframe.h1]!;
    final c4h = tfCandles[Timeframe.h4]!;
    ...
    final cl4h = c4h.map((c) => c.close).toList();
    final kama4h = Indicators.kama(cl4h, p.kamaPeriod);
    final adx4h = Indicators.adx(c4h, 14);

    final trades = <Trade>[];
    final s = _Sim();

    for (int i = max(p.kamaPeriod + 20, 60); i < c15.length - 1; i++) {
      if (e9_15[i].isNaN ||
          e21_15[i].isNaN ||
          atr15[i].isNaN ||
          volSma15[i].isNaN)
      ...
      final i1h = Indicators.closedHtfIdx(c1h, t);
      final i4h = Indicators.closedHtfIdx(c4h, t);
      if (i1h < 0 || i4h < 5) continue;
      if (rsi1h[i1h].isNaN || kama4h[i4h].isNaN || kama4h[i4h - 5].isNaN)
        continue;
      ...
      if (!s.inTrade) {
        final adxOk = adx4h.adx[i4h] > p.adxMin;
        final volOk = c15[i].volume > p.volFactor * volSma15[i];
        final kamaUp = kama4h[i4h] > kama4h[i4h - 5];
        final kamaDown = kama4h[i4h] < kama4h[i4h - 5];
        final rsiVal = rsi1h[i1h];
        final rsiL = rsiVal >= p.rsiLow && rsiVal <= p.rsiHigh;
        final rsiS = rsiVal >= (100 - p.rsiHigh) && rsiVal <= (100 - p.rsiLow);
        final crossL = e9_15[i] > e21_15[i] && e9_15[i - 1] <= e21_15[i - 1];
        final crossS = e9_15[i] < e21_15[i] && e9_15[i - 1] >= e21_15[i - 1];

        if (adxOk && volOk && kamaUp && rsiL && crossL) {
          s.dir = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
          ...
          s.entryBar = i + 1;
          s.beSet = false;
        } else if (adxOk && volOk && kamaDown && rsiS && crossS) {
          s.dir = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
...
class ChandelierMtf extends Strategy {
  @override
  String get name => 'Chandelier-MTF';
  ...
  List<Trade> backtest({
    ...
    final c1h = tfCandles[Timeframe.h1]!;
    final c4h = tfCandles[Timeframe.h4]!;
    final c1d = tfCandles[Timeframe.d1]!; // v9: daily for EMA slope gate
    ...
    // v9 key addition: daily EMA20 slope gate — only long when daily trend is up
    final e20_1d = Indicators.ema(c1d.map((c) => c.close).toList(), 20);
    ...
    for (int i = p.chPeriod + 10; i < c15.length - 1; i++) {
      ...
      if (!s.inTrade) {
        final adxOk = adx4h.adx[i4h] > p.adxMin;
        final volOk = c15[i].volume > p.volFactor * volSma15[i];
        // v9: daily close vs EMA20 level — more stable than slope, captures
        // the primary trend direction without 1-bar slope noise
        final dBull = c1d[i1d].close > e20_1d[i1d];
        final dBear = c1d[i1d].close < e20_1d[i1d];
        ...
        if (adxOk && volOk && dBull && d4h == 1 && flipL && conf15L) {
          s.dir = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
          ...
          s.entryBar = i + 1;
          s.beSet = false;
        } else if (adxOk && volOk && dBear && d4h == -1 && flipS && conf15S) {
          s.dir = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
...
// ════════════════════════════════════════════════════════════════════════════
// STRATEGY 5 — MOMENTUM TREND  (NEW in v9)
// ════════════════════════════════════════════════════════════════════════════
//
//  Concept  Catch assets mid-trend as momentum accelerates.
//           Unlike ChandelierMtf (which waits for a 1h flip), this strategy
//           uses the 1h RSI crossing a momentum threshold as the trigger.
//           The ADX must be RISING (not just above a level) to confirm the
//           trend is genuinely strengthening, not fading.
//
//  Entry    4h chandelier direction (trend filter).
//           4h ADX > threshold AND ADX[i] > ADX[i-2] (rising over 2 bars).
//           Daily EMA20 slope confirms primary direction.
//           1h RSI crosses UP through rsiLevel (for longs) — momentum entry.
//           1h volume > 1.5× SMA20 on signal bar.
//           15m chandelier direction confirms.
//  Exit     Fixed target; chandelier trailing stop; break-even; time stop.
// ════════════════════════════════════════════════════════════════════════════

class MomentumTrendParams extends StrategyParams {
  final int adxMin;
  final double rsiLevel; // RSI level to cross (e.g. 52, 55)
  @override
  final double atrStopMult, atrTargetMult;
  MomentumTrendParams(
    this.adxMin,
  ...
  @override
  String get label =>
      'MTrend ADX>$adxMin RSI×${rsiLevel.toStringAsFixed(0)} SL×$atrStopMult TP×$atrTargetMult';
}

class MomentumTrend extends Strategy {
  @override
  String get name => 'MomentumTrend';

  @override
  List<StrategyParams> grid() {
    final out = <StrategyParams>[];
    for (final adx in [20, 25, 30]) {
      for (final rsiLvl in [52.0, 55.0]) {
        for (final rr in [3.0, 4.0]) {
          out.add(MomentumTrendParams(adx, rsiLvl, 2.0, 2.0 * rr));
        }
      }
    }
  ...
  List<Trade> backtest({
    ...
    final p = params as MomentumTrendParams;
    final c15 = tfCandles[Timeframe.m15]!;
    final c1h = tfCandles[Timeframe.h1]!;
    final c4h = tfCandles[Timeframe.h4]!;
    final c1d = tfCandles[Timeframe.d1]!;

    // 4h: trend direction + ADX strength
    final ch4h = Indicators.chandelier(c4h, 14, 2.5);
    final adx4h = Indicators.adx(c4h, 14);

    // 1h: RSI cross trigger + volume
    final cl1h = c1h.map((c) => c.close).toList();
    final rsi1h = Indicators.rsi(cl1h, 14);
    final vol1h = c1h.map((c) => c.volume).toList();
    final volSma1h = Indicators.sma(vol1h, 20);

    // 15m: confirmation + stop management
    final ch15 = Indicators.chandelier(c15, 14, 2.5);
    final atr15 = Indicators.atr(c15, 14);
    final cl15 = c15.map((c) => c.close).toList();
    final e20_15 = Indicators.ema(cl15, 20);
    ...
    for (int i = 60; i < c15.length - 1; i++) {
      if (atr15[i].isNaN || e20_15[i].isNaN) continue;
      final t = c15[i].time;
      final i1h = Indicators.closedHtfIdx(c1h, t);
      final i4h = Indicators.closedHtfIdx(c4h, t);
      final i1d = Indicators.closedHtfIdx(c1d, t);
      if (i1h < 2 || i4h < 2 || i1d < 1) continue;
      if (rsi1h[i1h].isNaN || rsi1h[i1h - 1].isNaN) continue;
      if (adx4h.adx[i4h].isNaN || adx4h.adx[i4h - 2].isNaN) continue;
      if (e20_1d[i1d].isNaN || e20_1d[i1d - 1].isNaN) continue;
      if (volSma1h[i1h].isNaN) continue;

      final barRange = c15[i].high - c15[i].low;
      if (barRange > 1.5 * atr15[i]) continue;

      if (!s.inTrade) {
        // 4h trend direction via chandelier
        final d4h = ch4h.dir[i4h];
        // 4h ADX strong AND rising (trend has momentum AND is accelerating)
        final adxStrong = adx4h.adx[i4h] > p.adxMin;
        final adxRising = adx4h.adx[i4h] > adx4h.adx[i4h - 2];
        // Daily slope gate — must agree with entry direction
        final dBull = c1d[i1d].close > e20_1d[i1d];
        final dBear = c1d[i1d].close < e20_1d[i1d];
        // 1h RSI momentum cross (the actual entry trigger)
        final rsiCrossL =
            rsi1h[i1h] > p.rsiLevel && rsi1h[i1h - 1] <= p.rsiLevel;
        final rsiCrossS =
            rsi1h[i1h] < (100 - p.rsiLevel) &&
            rsi1h[i1h - 1] >= (100 - p.rsiLevel);
        // 1h volume confirms the momentum signal
        final volOk = c1h[i1h].volume > 1.5 * volSma1h[i1h];
        // 15m chandelier direction — final confirmation
        final d15 = ch15.dir[i];

        if (adxStrong && adxRising && dBull && d4h == 1 && rsiCrossL && volOk && d15 == 1) {
          s.dir = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
          ...
          s.entryBar = i + 1;
          s.beSet = false;
        } else if (adxStrong && adxRising && dBear && d4h == -1 && rsiCrossS && volOk && d15 == -1) {
          s.dir = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
...
class WfoEngine {
  ...
  WfoEngine({
    this.isLengthDays = 60,  // v9: same as v6 — sufficient IS data
    this.oosLengthDays = 30, // v9: 30-day OOS windows, covers all data via sliding
    this.minIsTrades = 8,    // v9: slightly lower — strategies are more selective
  });

  List<WalkForwardWindow> _windows(DateTime start, DateTime end) {
    // v9: sliding WFO windows (IS=60d, OOS=30d) covering all available data.
    // This adapts params every 30 days and produces consolidated OOS results
    // across the full data range — regime-adaptive, no single-split limitation.
    ...
    while (true) {
      ...
      // Clamp the last window to end of data instead of skipping it
      final oosEndClamped = oosEnd.isAfter(end) ? end : oosEnd;
      if (!isEnd.isBefore(end)) break; // not even enough data for IS
      wins.add(
        WalkForwardWindow(
          isStart: isStart,
          isEnd: isEnd,
          oosStart: isEnd,
          oosEnd: oosEndClamped,
          index: idx++,
        ),
      );
      if (oosEnd.isAfter(end)) break; // last window already clamped
  ...
  Map<String, List<StrategyResult>> runAll({
    ...
    print('  → ${wins.length} WFO windows (IS=${isLengthDays}d / OOS=${oosLengthDays}d sliding, covers all data)');
...
class Reporter {
  ...
  static void banner() {
    print('''
╔══════════════════════════════════════════════════════════════════════════════╗
║   CRYPTO TREND-FOLLOWING · v9                                                 ║
║   Sliding WFO (IS=60d / OOS=30d) covering ALL available data                  ║
║   Chandelier-MTF: +daily EMA slope gate (primary trend filter)                ║
║   TrendPullback-EMA: +1h EMA alignment + 1.3x volume + tighter RSI zone       ║
║   NEW: MomentumTrend (ADX level + 1h RSI cross trigger + 1h volume)           ║
║   IS gate: expectancy > 0 AND trades >= 8                                     ║
╚══════════════════════════════════════════════════════════════════════════════╝
''');
  }
  ...
  static void strategyCard(StrategyResult r) {
    ...
    print('\n  Exit reasons:');
    for (final e
        in reasons.entries.toList()..sort((a, b) => b.value.compareTo(a.value)))
...
''');
  ...
  static void exportCsv(
    ...
    final lb = File('$outDir/leaderboard_v9.csv');
    ...
    final tr = File('$outDir/all_trades_v9.csv');
    ...
    final wb = File('$outDir/window_breakdown_v9.csv');
    ...
    print('  📄 leaderboard_v9.csv');
    print('  📄 all_trades_v9.csv');
    print('  📄 window_breakdown_v9.csv');
...
void main(List<String> args) {
  ...
  final outDir = args.length > 1 ? args[1] : './wf_results_v9';
  ...
  print('  OOS window     : $oosLengthDays days  (sliding, covers ALL data)');
  ...
  // v9 strategies: proven v6 strategies with stronger filters + new MomentumTrend.
  ...
  final engine = WfoEngine(
    ...
    minIsTrades: 8, // v9: slightly lower — stricter filters reduce IS trade count
  );
