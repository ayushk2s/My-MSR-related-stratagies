...
class ChandelierMtf extends Strategy {
  ...
  List<Trade> backtest({
    ...
    // ── 4h indicators (trigger + ADX strength) ──────────────────────────────
    ...
    for (int i = p.chPeriod + 10; i < c1h.length - 1; i++) {
      ...
      if (!s.inTrade) {
        ...
        final trend4hL = d4h == 1;   // 4h is in bullish state
        final trend4hS = d4h == -1;  // 4h is in bearish state

        // Confirm: 1h close on correct side of EMA50
        final conf1hL = c1h[i].close > e50_1h[i];
        final conf1hS = c1h[i].close < e50_1h[i];

        if (adxOk && volOk && dBull && flip1hL && trend4hL && conf1hL) {
          s.dir        = TradeDirection.long;
          ...
          s.entryBar   = i + 1;
          s.beSet      = false;
        } else if (adxOk && volOk && dBear && flip1hS && trend4hS && conf1hS) {
          s.dir        = TradeDirection.short;
          s.entryPrice = _entryFill(c1h[i + 1].open, s.dir);
          s.origStop   = s.entryPrice + p.atrStopMult * atr1h[i];
          s.stopLoss   = s.origStop;
...
class Reporter {
  ...
  static void banner() {
    print('''
╔══════════════════════════════════════════════════════════════════════════════╗
║   CRYPTO TREND-FOLLOWING · v10  (QUALITY-FIRST REBUILD)                       ║
║   Single strategy: Chandelier-MTF upgraded to 1h entry timeframe              ║
║   Trigger: 4h chandelier FLIP  |  Confirm: 1h state + price > EMA50           ║
...
''');
  ...
  static void exportCsv(
    ...
    final lb = File('$outDir/leaderboard_v10.csv');
    ...
    final tr = File('$outDir/all_trades_v10.csv');
    ...
    final wb = File('$outDir/window_breakdown_v10.csv');
    ...
    print('  📄 leaderboard_v10.csv');
    print('  📄 all_trades_v10.csv');
    print('  📄 window_breakdown_v10.csv');
...
void main(List<String> args) {
  ...
  final outDir = args.length > 1 ? args[1] : './wf_results_v10';
  ...
  // v10: single strategy — Chandelier-MTF upgraded to 1h entry.
