  ...
  List<Trade> backtest({
    ...
    for (int i = p.chPeriod + 10; i < c15.length - 1; i++) {
      ...
      if (!s.inTrade) {
        ...
        if (adxOk && volOk && d4h == 1 && flipL && conf15L) {
          s.dir = TradeDirection.long;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice - p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
          ...
          s.entryBar = i + 1;
          s.beSet = false;
        } else if (adxOk && volOk && d4h == -1 && flipS && conf15S) {
          s.dir = TradeDirection.short;
          s.entryPrice = _entryFill(c15[i + 1].open, s.dir);
          s.origStop = s.entryPrice + p.atrStopMult * atr15[i];
          s.stopLoss = s.origStop;
...
class WfoEngine {
  ...
  List<WalkForwardWindow> _windows(DateTime start, DateTime end) {
    // Full backtest mode: ONE window only.
    // IS = first isLengthDays days (param selection).
    // OOS = all remaining data up to the last candle.
    final isEnd = start.add(Duration(days: isLengthDays));
    if (!isEnd.isBefore(end)) return []; // not enough data even for IS
    return [
      WalkForwardWindow(
        isStart: start,
        isEnd: isEnd,
  ...
  Map<String, List<StrategyResult>> runAll({
    ...
    print('  → Full backtest: IS=first 60d, OOS=all remaining (${wins[0].oosEnd.difference(wins[0].oosStart).inDays}d)');
...
class Reporter {
  ...
  static void banner() {
    print('''
╔══════════════════════════════════════════════════════════════════════════════╗
║   CRYPTO TREND-FOLLOWING · FULL BACKTEST (based on v6)                        ║
║   IS = first 60 days (param selection only)                                   ║
║   OOS = ALL remaining data (~275 days) — single split, no sliding windows     ║
...
''');
  ...
  static void exportCsv(
    ...
    final lb = File('$outDir/leaderboard_fullbt.csv');
    ...
    final tr = File('$outDir/all_trades_fullbt.csv');
    ...
    final wb = File('$outDir/window_breakdown_fullbt.csv');
    ...
    print('  📄 leaderboard_fullbt.csv');
    print('  📄 all_trades_fullbt.csv');
    print('  📄 window_breakdown_fullbt.csv');
...
void main(List<String> args) {
  ...
  final outDir = args.length > 1 ? args[1] : './wf_results_fullbt';

  const int isLengthDays = 60;
  const int oosLengthDays = 9999; // full backtest: OOS = all remaining data

  print('Configuration');
  print('  Data dir       : $dataDir');
  print('  Output dir     : $outDir');
  print('  IS window      : $isLengthDays days  (param optimisation)');
  print('  OOS window     : all remaining data after IS (single split)');
  ...
  // Full backtest: only the two proven v6 strategies.
  // SupertrendQuality and KamaPullback dropped — underperformed in WFO.
