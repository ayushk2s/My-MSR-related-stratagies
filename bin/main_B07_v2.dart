// =============================================================================
// V7 LIVE TRADING BOT  —  Exact port of v7_portfolio_sim.dart
// =============================================================================
// Strategy : E15m / T45m / SR(15m+30m) / SFI(5,1.2) / prox0.8% /
//            RR≥1.0 / TP0.70 / SL0.20%buf / EMA100 on 45m / 6-bar cooldown
// Assets   : BCH BNB BTC DOGE ETC ETH LINK SOL XRP (all 9 from backtest)
// Validated: OOS CAGR +37% | MaxDD 11.6% | Calmar 3.18
//            5/5 years profitable | 8/9 assets positive
// =============================================================================
...
/// Exponential moving average — exact copy of backtest `_emaList()`
List<double> _computeEma(List<Candle> cs, int p) {
  final out = <double>[];
  if (cs.isEmpty) return out;
  final mul = 2.0 / (p + 1);
  double ema = cs[0].close;
  for (final c in cs) { ema = c.close * mul + ema * (1 - mul); out.add(ema); }
  return out;
}
...
class V7Bot {
  ...
  Future<void> _tick() async {
    ...
    try {
      ...
      final ema45List = _computeEma(c45, _emaPeriod);
      ...
      if (isNew15m) {
        ...
        if (_trade == null && _pendingDir == null && _cooldownLeft == 0) {
          _detectSignal(
            sfi15Cur, sfi45Cur, ema45, price, allSup, allRes,
          );
        }
      }
  ...
  // ─── Signal Detection ───────────────────────────────────────────────────────
  //
  // Exact port of backtest "Signal detection" block:
  //   1. SFI(5,1.2) on 15m — detect buy/sell flip
  //   2. 45m SFI trend filter (bearish blocks buy, bullish blocks sell)
  //   3. 45m EMA(100) filter  (below EMA blocks buy, above EMA blocks sell)
  //   4. SR proximity 0.8%   (must be near a known S or R zone)

  void _detectSignal(
    _SfiSig sfi15, _SfiSig sfi45, double ema45,
    ...
    // Filter 2 — 45m EMA(100)
    if (buyFlip  && price < ema45) {
      print('[$symbol]   BUY blocked: price ${price.toStringAsFixed(4)} < EMA ${ema45.toStringAsFixed(4)}');
      buyFlip = false;
    }
    if (sellFlip && price > ema45) {
      print('[$symbol]   SELL blocked: price ${price.toStringAsFixed(4)} > EMA ${ema45.toStringAsFixed(4)}');
      sellFlip = false;
    }
    if (!buyFlip && !sellFlip) return;

    // Filter 3 — SR proximity (0.8%)
    if (buyFlip) {
  ...
  Future<void> _closeRemaining(_V7Trade t, String reason) async {
    try {
      final side    = t.dir == 1 ? 'SELL' : 'BUY';
      final posSide = t.dir == 1 ? 'LONG' : 'SHORT';
...
║  Trend   : 45m SFI trend + EMA($emaPeriod)
...
// =============================================================================
// MAIN  —  All 9 V7-validated assets
// =============================================================================
...
Future<void> main() async {
  ...
  // V7 backtest assets — all 9 that were swept.
  // (BNB was excluded only from B07 sweep; V7 sim includes it.)
...
final bots = assets.map((asset) => V7Bot(
  apiKey: apiKey,
  secretKey: secretKey,
  symbol: asset.$1,
  ...
  print('\n[MAIN] ${bots.length} V7 bots running'
