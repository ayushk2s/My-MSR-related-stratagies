...
class TradingBot {
  ...
  Future<void> _tick() async {
    ...
    try {
      ...
      // Sort by boxRight descending — use the zone most recently touched (highest bar index)
      ...
      final s0 = activeSupports.isNotEmpty
          ? 'S=${activeSupports.first.boxBottom.toStringAsFixed(4)}-${activeSupports.first.boxTop.toStringAsFixed(4)}(bar${activeSupports.first.boxRight})'
          : 'S=none';
      final r0 = activeResistances.isNotEmpty
          ? 'R=${activeResistances.first.boxBottom.toStringAsFixed(4)}-${activeResistances.first.boxTop.toStringAsFixed(4)}(bar${activeResistances.first.boxRight})'
          : 'R=none';
      ...
      // 5. Check for new entries — only ACTIVE zones (broken zones excluded)
  ...
  Future<void> _manageOpenTrades(SfiSignal sfi, Candle lastCandle) async {
    ...
    for (final trade in _openTrades) {
      // Update trailing SL — after TP1, clamp so SL never goes back through entry (breakeven floor)
      ...
      // Mark targets reached (for display of targets 2-4)
      ...
      if (trade.direction == TradeDirection.long) {
        ...
        if (!trade.tp1Hit && trade.targets.isNotEmpty && lastCandle.high >= trade.targets[0].price) {
          ...
          if (closeQty > 0) {
            ...
            print('[TRADE] LONG #${trade.id} TP1 60% @ ${trade.targets[0].price.toStringAsFixed(4)}'
        ...
        // ── TP2: close 30% at target[1] (remaining becomes ~10%) ──
        if (trade.tp1Hit && !trade.tp2Hit && trade.targets.length > 1 && lastCandle.high >= trade.targets[1].price) {
          ...
          if (actualClose > 0) {
            ...
            trade.realizedPnl += closePnl;
            print('[TRADE] LONG #${trade.id} TP2 30% @ ${trade.targets[1].price.toStringAsFixed(4)}'
        ...
        // ── SFI flip: close all remaining ──
        if (sfi.sellSignal) {
          final closePnl = (lastCandle.close - trade.entryPrice) * trade.remainingQty;
          trade.realizedPnl += closePnl;
          print('[TRADE] LONG #${trade.id} EXIT (SFI FLIP) @ ${lastCandle.close.toStringAsFixed(4)}'
              ' remaining=${trade.remainingQty} PnL ${closePnl >= 0 ? "+" : ""}${closePnl.toStringAsFixed(4)}');
        ...
        if (lastCandle.low <= trade.currentSl) {
          ...
          trade.realizedPnl += closePnl;
          print('[TRADE] LONG #${trade.id} SL @ ${trade.currentSl.toStringAsFixed(4)}'
              ' remaining=${trade.remainingQty} PnL ${closePnl >= 0 ? "+" : ""}${closePnl.toStringAsFixed(4)}');
        ...
        // ── TP1: close 60% at target[0] ──
        if (!trade.tp1Hit && trade.targets.isNotEmpty && lastCandle.low <= trade.targets[0].price) {
          final closeQty = _roundQty(trade.totalQty * 0.60);
          if (closeQty > 0) {
            final closePnl = (trade.entryPrice - trade.targets[0].price) * closeQty;
            trade.realizedPnl += closePnl;
            print('[TRADE] SHORT #${trade.id} TP1 60% @ ${trade.targets[0].price.toStringAsFixed(4)}'
                ' qty=$closeQty PnL +${closePnl.toStringAsFixed(4)}');
            await _exitPosition(trade: trade, closeQty: closeQty, reason: 'TP1_60pct');
            trade.remainingQty = _roundQty(trade.remainingQty - closeQty);
            trade.tp1Hit = true;
        ...
        // ── TP2: close 30% at target[1] (remaining becomes ~10%) ──
        if (trade.tp1Hit && !trade.tp2Hit && trade.targets.length > 1 && lastCandle.low <= trade.targets[1].price) {
          final closeQty = _roundQty(trade.totalQty * 0.30);
          final actualClose = closeQty <= trade.remainingQty ? closeQty : trade.remainingQty;
          if (actualClose > 0) {
            final closePnl = (trade.entryPrice - trade.targets[1].price) * actualClose;
            trade.realizedPnl += closePnl;
            print('[TRADE] SHORT #${trade.id} TP2 30% @ ${trade.targets[1].price.toStringAsFixed(4)}'
                ' qty=$actualClose PnL +${closePnl.toStringAsFixed(4)}');
            await _exitPosition(trade: trade, closeQty: actualClose, reason: 'TP2_30pct');
            trade.remainingQty = _roundQty(trade.remainingQty - actualClose);
            trade.tp2Hit = true;
        ...
        if (sfi.buySignal) {
          ...
          trade.realizedPnl += closePnl;
          print('[TRADE] SHORT #${trade.id} EXIT (SFI FLIP) @ ${lastCandle.close.toStringAsFixed(4)}'
              ' remaining=${trade.remainingQty} PnL ${closePnl >= 0 ? "+" : ""}${closePnl.toStringAsFixed(4)}');
          await _exitPosition(trade: trade, closeQty: trade.remainingQty, reason: 'SFI_FLIP');
          trade.remainingQty = 0;
          _recordClosedTrade(trade, lastCandle.close);
        ...
        if (lastCandle.high >= trade.currentSl) {
          ...
          trade.realizedPnl += closePnl;
          print('[TRADE] SHORT #${trade.id} SL @ ${trade.currentSl.toStringAsFixed(4)}'
              ' remaining=${trade.remainingQty} PnL ${closePnl >= 0 ? "+" : ""}${closePnl.toStringAsFixed(4)}');
          await _exitPosition(trade: trade, closeQty: trade.remainingQty, reason: 'SL');
          trade.remainingQty = 0;
          _recordClosedTrade(trade, trade.currentSl);
  ...
  Future<void> _checkEntries(
    ...
    // ── LONG: SFI flips to +1 + candle touches S[0] ──
    if (currentSfi.buySignal && _openTrades.isEmpty && activeSupports.isNotEmpty) {
      ...
      final touchesSupport = zoneWidthPct >= 0.003 &&
          lastCandle.low >= zone.boxBottom && lastCandle.low <= zone.boxTop;
    ...
    // ── SHORT: SFI flips to -1 + candle touches R[0] ──
    if (currentSfi.sellSignal && _openTrades.isEmpty && activeResistances.isNotEmpty) {
      ...
      final touchesResistance = zoneWidthPct >= 0.003 &&
          lastCandle.high >= zone.boxBottom && lastCandle.high <= zone.boxTop;
...
Future<void> main() async {
  ...
  final bots = symbols.map((sym) => TradingBot(BotConfig(
    ...
    quantityPrecision: 3, // auto-fetched at startup per symbol
  ...
  // Stagger starts by 2s so they don't all hit the API simultaneously
  for (final bot in bots) {
    bot.start();
    await Future.delayed(Duration(seconds: 2));
  }

  print('[MAIN] All bots running (${symbols.join(", ")}). Press Ctrl+C to stop.\n');
