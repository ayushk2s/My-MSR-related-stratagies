...
class BacktestConfig {
  ...
  // Only enter when |entry − zone boundary| / entry * 100 < this threshold.
  // Keeps entries where price barely crossed the zone edge → tight stop,
  // smaller potential loss per trade, but fewer signals overall.
  // 0.0 = disabled.  e.g. 0.3 = only take entries within 0.3% of zone boundary.
  final double maxStopDistPct;
  ...
  List<BtTrade> run(List<Candle> allCandles, {List<Candle>? htfCandles}) {
    ...
    for (int i = config.warmupBars; i < totalBars; i++) {
      ...
      void openTrade({required bool isLong, required SRZone zone}) {
        ...
        // Filter 5: maximum stop distance — only enter when stop is TIGHT
        // i.e. entry price is within maxStopDistPct% of the zone boundary.
        // Smaller stop = less risk per trade if it goes wrong.
        if (config.maxStopDistPct > 0 && cStopDist > config.maxStopDistPct) return;
...
class Reporter {
  ...
  void _printInvestorReport() {
    ...
    _bxkv(bw, 'Max Stop Dist',   config.maxStopDistPct > 0
        ? '<${config.maxStopDistPct}% from entry  (tight stop filter)'
        : 'disabled');
    print('╠${'═' * bw}╣');
...
void main() {
  final config = BacktestConfig(
    ...
    // ── Multi-timeframe SR ──────────────────────────────────────────────────
    htfCsvPath:      '/Users/ayush/Desktop/candlestick data/5m/SOLUSDT5m.csv',
    ...
    // ── Maximum stop distance filter ─────────────────────────────────────────
    // Only enter when entry is within 0.3% of the zone boundary (tight stop).
    // Thesis: entry barely touched zone edge = higher R:R, smaller loss if wrong.
    maxStopDistPct: 0.3,
  );
