// =============================================================================
// STRATEGY HUNT V4 — SR + SFI + Extra Indicators, Multi-Timeframe
//                    REAL 5m data (no simulated candles)
// =============================================================================
//
...
// _sim5m removed in V4 — real 5m CSV data is loaded directly
  ...
  for (int i = 1; i < c5.length; i++) {
    ...
    if (active == null && cd == 0 && !pendL && !pendS) {
      ...
      if (s.entryTfMin == 5) {
        ...
        // We're at 5m bar i; check if this is the LAST 5m bar of a 15m bar (i%3 == 2)
        buyFlip  = (i % 3 == 2) && (i15 > 0 ? sfi15[i15].buy  : false);
        sellFlip = (i % 3 == 2) && (i15 > 0 ? sfi15[i15].sell : false);
...
void main() {
  const base5m = '/Users/ayush/Desktop/candlestick data/5m';
  const srLen45 = 10, srLen15 = 12, srLen5 = 8;

  final assets = [
    ('SOLUSDT',  '$base5m/SOLUSDT5m.csv'),
    ('XRPUSDT',  '$base5m/XRPUSDT5m.csv'),
    ('TRBUSDT',  '$base5m/TRBUSDT5m.csv'),
    ('BTCUSDT',  '$base5m/BTCUSDT5m.csv'),
    ('ETHUSDT',  '$base5m/ETHUSDT5m.csv'),
    ('DOGEUSDT', '$base5m/DOGEUSDT5m.csv'),
    ('BNBUSDT',  '$base5m/BNBUSDT5m.csv'),
  ];

  final allRes = <Res>[];

  for (final (sym, path) in assets) {
    if (!File(path).existsSync()) { print('SKIP $sym'); continue; }
    stdout.write('  $sym ... ');

    // V4: load real 5m data, aggregate up to 15m and 45m
