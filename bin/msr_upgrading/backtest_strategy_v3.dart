...
List<Candle> _sim5m(List<Candle> c15) {
  ...
  for (final c in c15) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      out.add(Candle(c.time.add(Duration(minutes: 5 * i)), o5, h5, l5, c5, c.volume / 3.0, out.length));
    }
  }
  return out;
}
...
void main() {
  ...
  final assets = [
    ('SOLUSDT',  '$base/SOLUSDT15m.csv'),
    ('XRPUSDT',  '$base/XRPUSDT15m.csv'),
    ('TRBUSDT',  '$base/TRBUSDT15m.csv'),
    ('BTCUSDT',  '$base/BTCUSDT15m.csv'),
    ('ETHUSDT',  '$base/ETHUSDT15m.csv'),
    ('DOGEUSDT', '$base/DOGEUSDT15m.csv'),
    ('BNBUSDT',  '$base/BNBUSDT15m.csv'),
  ];
  ...
  for (final (sym, path) in assets) {
    ...
    final c15 = _clean(_loadCsv(path), 15);
