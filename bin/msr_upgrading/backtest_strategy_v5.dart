// =============================================================================
// STRATEGY V5 — Exhaustive Multi-TF Sweep
// Timeframes : 5m · 15m · 30m · 45m  (1m/3m skipped — no CSV available)
// Assets     : 21 (all 5m CSVs)
// Strategies : ~360 auto-generated (Entry TF × Trend TF × SR TF × SFI × Filter)
// =============================================================================
// Strategy params:
//   Entry TF  : 5m | 15m | 30m
//   Trend TF  : none | 15m | 30m | 45m  (must be > entry)
//   SR zones  : entry TF only | trend TF only | both
//   SFI       : (7,1.5) | (10,1.7) | (14,2.0)
//   Filter    : none | rej-candle | R:R≥1.5 | R:R≥2.0 | EMA200(45m)
//
// Bias-free:  signal[i-1], fill next-bar open, SL wins same-bar
...
class Strat {
  ...
  const Strat(this.name, this.entryTf, this.trendTf, this.srMask,
              this.sfiP, this.sfiM, this.rej, this.minRR, this.ema);
}
...
List<Strat> _genStrategies() {
  ...
  // Filter combos: (suffix, rej, minRR, ema)
  const filters = <(String, bool, double, bool)>[
    ('',       false, 0.0, false),
    ('+Rej',   true,  0.0, false),
    ('+RR1.5', false, 1.5, false),
    ('+RR2.0', false, 2.0, false),
    ('+EMA',   false, 0.0, true),
  ];
...
class AssetData {
  ...
  // SFI arrays keyed by "period_mult" -> TF map
  final Map<String, List<SfiSig>> sfi5Map, sfi15Map, sfi30Map, sfi45Map;
  final List<double> atr5, ema200_45;
  ...
  AssetData({
    ...
    required this.atr5, required this.ema200_45,
...
AssetData _loadAsset(String sym, String path) {
  ...
  // SR zones per TF (computed once, reused across all strategies)
  List<SRZone> _zones(List<Candle> cs, int len) {
    final sr = SupportResistanceIndicator(
        detectionLength: len, srMargin: 2.0, avoidFBO: true, checkHist: true);
...
Res _run(AssetData d, Strat s) {
  ...
  for (int i = 1; i < c5.length; i++) {
    ...
    List<SRZone> getSup() {
      ...
      allSup = <SRZone>[];
      if (s.srMask & _B5  != 0) allSup!.addAll(_knSup(d.zones5,  i,   _srl5));
      if (s.srMask & _B15 != 0) allSup!.addAll(_knSup(d.zones15, i15, _srl15));
      if (s.srMask & _B30 != 0) allSup!.addAll(_knSup(d.zones30, i30, _srl30));
      if (s.srMask & _B45 != 0) allSup!.addAll(_knSup(d.zones45, i45, _srl45));
      return allSup!;
    }
    List<SRZone> getRes() {
      if (allRes != null) return allRes!;
      allRes = <SRZone>[];
      if (s.srMask & _B5  != 0) allRes!.addAll(_knRes(d.zones5,  i,   _srl5));
      if (s.srMask & _B15 != 0) allRes!.addAll(_knRes(d.zones15, i15, _srl15));
      if (s.srMask & _B30 != 0) allRes!.addAll(_knRes(d.zones30, i30, _srl30));
      if (s.srMask & _B45 != 0) allRes!.addAll(_knRes(d.zones45, i45, _srl45));
      return allRes!;
    }
...
void main() {
  ...
  print('═' * 90);
  print(' STRATEGY V5 — ${_strategies.length} strategies × ${assetSet.length} assets'
  ...
  print('─' * 60);
  for (final mask in [_B5, _B15, _B30, _B45, _B15|_B45, _B15|_B30, _B30|_B45, _B5|_B15]) {
