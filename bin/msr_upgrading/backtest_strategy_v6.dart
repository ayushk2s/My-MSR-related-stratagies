// =============================================================================
// STRATEGY V6 — Exhaustive Multi-TF Sweep  *** FULLY BIAS-FREE ***
...
// =============================================================================
// V6 bias fixes vs V5:
//   FIX 1 — SR zone look-ahead (!s.b removed)
//     V5 used the indicator's final s.b flag which was set using FUTURE candles.
//     V6 computes breakBar per zone using only PAST closed candles:
//       - Resistance broken when close > boxTop (on any bar after zone visible)
//       - Support  broken when close < boxBottom
//     At bar i we only see zones whose breakBar > i (not broken yet as of bar i).
//
//   FIX 2 — checkHist: false
//     V5 used checkHist:true which may process historical confirmation
//     using future data.  V6 uses checkHist:false — zones formed strictly
//     from past pivot data only.
//
//   ISSUE 1 (same-bar SL/TP) — NOT changed:
//     Signal fires at candle CLOSE, fill at NEXT bar OPEN.
//     TP/SL on active trades: SL wins same bar = conservative (correct).
//     This never inflates results — it slightly hurts them.
//
