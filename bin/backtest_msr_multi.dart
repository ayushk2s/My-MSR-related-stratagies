...
class BacktestConfig {
  ...
  // Evidence: July is consistently negative across ADA, ETH, ETH, XRP, TRX.
  ...
  // Skip entry if |entry - zone boundary| / entry * 100 < this threshold.
  // Evidence from v1 backtest: >0.7% stop dist had 53.8% WR +$1785 vs 0.3-0.7% at 37.5% WR -$462.
  // 0.0 = disabled.
  final double minStopDistPct;
  ...
  List<BtTrade> run(List<Candle> allCandles, {List<Candle>? htfCandles}) {
    ...
    for (int i = config.warmupBars; i < totalBars; i++) {
      ...
      void openTrade({required bool isLong, required SRZone zone}) {
        ...
        // Filter 5: minimum stop distance — wide stops = stronger zone touch
        // Evidence: >0.7% had 53.8% WR vs 0.3-0.7% at 37.5% WR
        if (config.minStopDistPct > 0 && cStopDist < config.minStopDistPct) return;
...
class Reporter {
  ...
  void _printInvestorReport() {
    ...
    _bxkv(bw, 'Block Hours UTC', config.blockHours.isEmpty ? 'none' : config.blockHours.map((h) => '${h.toString().padLeft(2,"0")}:00').join(', '));
    _bxkv(bw, 'Min Stop Dist',   config.minStopDistPct > 0 ? '>${config.minStopDistPct}% from entry (zone depth filter)' : 'disabled');
    print('╠${'═' * bw}╣');
...
List<Candle> loadCsv(String path) {
  ...
  for (int i = 1; i < lines.length; i++) {   // skip header row
    ...
    try {
      final time   = DateTime.parse(parts[0] + 'Z'); // treat CSV timestamps as UTC
...
class _RunSummary {
  ...
  final String grade;
  const _RunSummary({
    required this.symbol, required this.htf,    required this.trades,
    required this.winRate, required this.pf,    required this.retPct,
    required this.maxDDPct, required this.calmar, required this.netPnl,
    required this.grade,
  });
}

void _printMasterSummary(List<_RunSummary> results) {
  // Sort: A grades first, then by return% descending
  final gradeOrder = {'A': 0, 'B': 1, 'C': 2, 'D': 3, 'N': 4};
  results.sort((a, b) {
    final ga = gradeOrder[a.grade[0]] ?? 5;
    final gb = gradeOrder[b.grade[0]] ?? 5;
    if (ga != gb) return ga.compareTo(gb);
    return b.retPct.compareTo(a.retPct);
  });
  ...
  print('\n\n$sep');
  print('  MASTER SUMMARY  —  All Assets  ×  Both HTF Timeframes');
  print('  Sorted by Grade then Return%');
  print(sep);
  print(
    '  ${'Symbol'.padRight(10)}'
    '${'HTF'.padRight(6)}'
  ...
  for (final r in results) {
    final g = r.grade[0];
    if (g != lastGrade) { print(dash); lastGrade = g; }

    final noTrades = r.trades == 0;
    print(
      '  ${r.symbol.padRight(10)}'
      '${r.htf.padRight(6)}'
      '${r.trades.toString().padLeft(7)}'
      '${noTrades ? "     —" : "${r.winRate.toStringAsFixed(1)}%".padLeft(8)}'
      '${noTrades ? "     —" : r.pf.toStringAsFixed(2).padLeft(6)}'
      '${noTrades ? "         —" : "${r.retPct >= 0 ? "+" : ""}${r.retPct.toStringAsFixed(2)}%".padLeft(10)}'
      '${noTrades ? "       —" : "-${r.maxDDPct.toStringAsFixed(2)}%".padLeft(8)}'
      '${noTrades ? "       —" : r.calmar.toStringAsFixed(2).padLeft(8)}'
      '${noTrades ? "           —" : "${r.netPnl >= 0 ? "+" : ""}\$${r.netPnl.toStringAsFixed(2)}".padLeft(12)}'
      '  ${r.grade}',
    );
  }
  print(sep);

  // Best per symbol (whichever HTF performed better)
  final symbols = results.map((r) => r.symbol).toSet().toList()..sort();
  print('\n  BEST HTF PER SYMBOL');
  print(dash);
  print(
    '  ${'Symbol'.padRight(10)}'
    '${'Best HTF'.padRight(10)}'
    '${'Return%'.padLeft(10)}'
    '${'Calmar'.padLeft(8)}'
    '${'WR%'.padLeft(8)}'
    '  Grade',
  ...
  for (final sym in symbols) {
    final runs = results.where((r) => r.symbol == sym).toList();
    if (runs.isEmpty) continue;
    final best = runs.reduce((a, b) => a.retPct > b.retPct ? a : b);
    print(
      '  ${best.symbol.padRight(10)}'
      '${best.htf.padRight(10)}'
      '${"${best.retPct >= 0 ? "+" : ""}${best.retPct.toStringAsFixed(2)}%".padLeft(10)}'
      '${best.calmar.toStringAsFixed(2).padLeft(8)}'
      '${best.trades > 0 ? "${best.winRate.toStringAsFixed(1)}%".padLeft(8) : "     —"}'
      '  ${best.grade}',
    );
  }
  print(sep);
...
BacktestConfig _buildConfig(String symbol, String csv5m, String csvHtf) =>
    BacktestConfig(
      csvPath:           csv5m,
      symbol:            symbol,
      sfiPeriod:         10,
...
// ============================================================================
// MAIN — multi-asset × multi-HTF loop
// ============================================================================

void main() {
  const baseDir = '/Users/ayush/Desktop/candlestick data';
  const htfList = ['15m', '30m'];

  // ── Auto-discover all assets from the 5m folder ───────────────────────────
  final dir5m  = Directory('$baseDir/5m');
  final files5m = dir5m.listSync()
      .whereType<File>()
      .where((f) => f.path.endsWith('5m.csv'))
      .toList()
        ..sort((a, b) => a.path.compareTo(b.path));

  final symbols = files5m.map((f) {
    final name = f.path.split('/').last;   // e.g.  BNBUSDT5m.csv
    return name.replaceAll('5m.csv', '');  // →     BNBUSDT
  }).toList();

  print('╔${'═' * 70}╗');
  print('║  MSR Multi-Asset Backtest'.padRight(71) + '║');
  print('║  Assets found : ${symbols.length}  (${symbols.join(', ')})'.padRight(71) + '║');
  print('║  HTF modes    : ${htfList.join(', ')}  →  ${symbols.length * htfList.length} total runs'.padRight(71) + '║');
  print('╚${'═' * 70}╝\n');

  final results = <_RunSummary>[];

  for (final symbol in symbols) {
    for (final htf in htfList) {
      final csv5m  = '$baseDir/5m/${symbol}5m.csv';
      final csvHtf = '$baseDir/$htf/$symbol$htf.csv';

      // ── Check files exist ─────────────────────────────────────────────────
      if (!File(csv5m).existsSync() || !File(csvHtf).existsSync()) {
        print('⚠ Skipping $symbol $htf — CSV not found\n');
        results.add(_RunSummary(symbol: symbol, htf: htf, trades: 0,
            winRate: 0, pf: 0, retPct: 0, maxDDPct: 0, calmar: 0,
            netPnl: 0, grade: 'N/A'));
        continue;
      }

      print('\n${'═' * 72}');
      print('  ▶  $symbol  │  HTF = $htf  │  loading data...');
      print('${'═' * 72}');

      final candles = loadCsv(csv5m);
      if (candles.isEmpty) {
        print('ERROR: no 5m candles for $symbol — skipping\n');
        results.add(_RunSummary(symbol: symbol, htf: htf, trades: 0,
            winRate: 0, pf: 0, retPct: 0, maxDDPct: 0, calmar: 0,
            netPnl: 0, grade: 'N/A'));
        continue;
      ...
      final htfCandles = loadCsv(csvHtf);
      if (htfCandles.isEmpty) {
        print('ERROR: no $htf candles for $symbol — skipping\n');
        results.add(_RunSummary(symbol: symbol, htf: htf, trades: 0,
            winRate: 0, pf: 0, retPct: 0, maxDDPct: 0, calmar: 0,
            netPnl: 0, grade: 'N/A'));
        continue;
      ...
      final winPnl  = trades.where((t) =>  t.isWin).fold(0.0, (s,t) => s+t.totalPnl);
      final lossPnl = trades.where((t) => !t.isWin).fold(0.0, (s,t) => s+t.totalPnl);
      final pf      = lossPnl.abs() > 0 ? winPnl / lossPnl.abs() : double.infinity;
      final retPct  = netPnl / config.initialBalance * 100;
      ...
      final maxDDPct = peak > 0 ? maxDD / peak * 100 : 0.0;
      final calmar   = maxDDPct > 0 ? retPct / maxDDPct : 0.0;

      final String grade;
      if (winRate >= 55 && pf >= 1.5 && retPct > 0)      grade = 'A ★★★  Excellent';
      else if (winRate >= 45 && pf >= 1.2 && retPct > 0) grade = 'B ★★   Good';
      else if (retPct > 0)                                grade = 'C ★    Marginal';
      else                                                grade = 'D      Losing';

      results.add(_RunSummary(
        symbol: symbol, htf: htf, trades: trades.length,
        winRate: winRate, pf: pf, retPct: retPct,
        maxDDPct: maxDDPct, calmar: calmar, netPnl: netPnl,
        grade: grade,
      ));
    }
  }

  // ── Print master ranking table ────────────────────────────────────────────
  _printMasterSummary(results);
}
